...
import os, sys             #Manages file paths and directories
import traceback           #Handles exceptions
import pyttsx3
from keras.models import load_model
from cvzone.HandTrackingModule import HandDetector #Detects hand landmark
...
from PIL import Image, ImageTk   #opening, editing, and saving images
...
class Application:
    ...
    def save_fun(self):
        text = self.panel5.cget("text").replace("Sentence :", "").strip()
        if not text:
            print("No sentence to save as audio!")
            return
            
        file_path = filedialog.asksaveasfilename(title="Save Audio File",
        ...
        if file_path:
            try:
                self.speak_engine.save_to_file(text, file_path)
    ...
    def translate_text(self):
        ...
        translated_text = translator.translate(text, dest=self.languages[language]).text
        self.result_label.config(text=f"Translated Text: {translated_text}")

        # Convert text to speech
        tts = gTTS(translated_text, lang=self.languages[language])
    ...
    def __init__(self):
        ...
        self.panel5 = tk.Label(self.root, wraplength=1000,font=("Arial", 12))  # Sentence
        self.panel5.place(x=240, y=640)
        ...
        self.b1.place(x=390,y=710)

        self.b2 = tk.Button(self.root)
        self.b2.place(x=590, y=710)

        self.b3 = tk.Button(self.root)
        self.b3.place(x=790, y=710)

        self.b4 = tk.Button(self.root)
        self.b4.place(x=990, y=710)

        self.speaker_icon = Image.open("speaker.png")  # Ensure speaker.png is in the same folder
        self.speaker_icon = self.speaker_icon.resize((40, 40), Image.Resampling.LANCZOS)
        self.speaker_photo = ImageTk.PhotoImage(self.speaker_icon)

        # Create a button with the speaker icon
        self.speak = tk.Button(
            self.root, image=self.speaker_photo, command=self.speak_fun, borderwidth=0
        )
        self.speak.place(x=1200, y=700)

        self.clear = tk.Button(self.root)
        self.clear.place(x=1250, y=450)
        self.clear.config(text="Clear", font=("Courier", 20), wraplength=100, command=self.clear_fun)

        self.save_audio = tk.Button(self.root)
        self.save_audio.place(x=1250, y=150)
        self.save_audio.config(text="Save Audio", font=("Courier", 20), wraplength=170, command=self.save_fun)

         # Translator UI
        self.languages = {"English": "en", "Spanish": "es", "French": "fr", "German": "de", "Hindi": "hi", "Chinese": "zh-cn","Bengali":"bn","Arabic":"ar","Gujarati":"gu","Kannada":"kn","Korean":"ko","Malayalam":"ml","Marathi":"mr","Punjabi":"pa","Russian":"ru","Tamil":"ta","Telugu":"te","Urdu":"ur"}
        ...
        self.language_dropdown.place(x=1250, y=530)

        self.translate_button = tk.Button(self.root, text="Translate", font=("Courier", 20), wraplength=200, command=self.translate_text)
        self.translate_button.place(x=1250, y=320)

        self.result_label = tk.Label(self.root, text="", wraplength=800, font=("Times New Roman", 10))

        self.result_label.place(x=900, y=590)
    ...
    def video_loop(self):#captures video frames, detects hands, processes hand landmarks, and displays 
        try:
            ok, frame = self.vs.read()  #Reads a frame from the video capture.
            cv2image = cv2.flip(frame, 1)  #Flips the frame horizontally to create a mirror effect.
            if cv2image.any:
                hands = hd.findHands(cv2image, draw=False, flipType=True)
                cv2image_copy=np.array(cv2image)
                cv2image = cv2.cvtColor(cv2image, cv2.COLOR_BGR2RGB)  #BGR to RGB
                ...
                if hands[0]:
                    ...
                    map = hand[0]
                    x, y, w, h=map['bbox']
                    image = cv2image_copy[y - offset:y + h + offset, x - offset:x + w + offset]   #Crops the hand region
                    ...
                    if image.all:
                        ...
                        if handz[0]:
                            hand = handz[0]
                            handmap=hand[0]
                            self.pts = handmap['lmList']  #Retrieves hand landmarks
                            # x1,y1,w1,h1=hand['bbox']

                            os = ((400 - w) // 2) - 15
                            os1 = ((400 - h) // 2) - 15
                            for t in range(0, 4, 1):  #Draws green lines between finger joints
                            ...
                            cv2.line(white, (self.pts[0][0] + os, self.pts[0][1] + os1), (self.pts[17][0] + os, self.pts[17][1] + os1), (0, 255, 0),
                                     3)  #Connects major hand landmarks 
                ...
                self.panel5.config(text=self.str, font=("Courier",20), wraplength=1500)
        except Exception:
            ...
            if hands:
                ...
                handz = hd2.findHands(image, draw=False, flipType=True)
                print(" ", self.ccc)
                self.ccc += 1 #increment each time hand detected
                if handz:
                    hand = handz[0]
                    self.pts = hand['lmList']
                    # x1,y1,w1,h1=hand['bbox']

                    os = ((400 - w) // 2) - 15  #center the hand on a 400x400 white background
                    os1 = ((400 - h) // 2) - 15
                    for t in range(0, 4, 1):
                        cv2.line(white, (self.pts[t][0] + os, self.pts[t][1] + os1), (self.pts[t + 1][0] + os, self.pts[t + 1][1] + os1),
                                 (0, 255, 0), 3)#Draws green lines connecting the first four finger joints
                    for t in range(5, 8, 1):
                        cv2.line(white, (self.pts[t][0] + os, self.pts[t][1] + os1), (self.pts[t + 1][0] + os, self.pts[t + 1][1] + os1),
                                 (0, 255, 0), 3)
                    for t in range(9, 12, 1):
    ...
    def predict(self, test_image):
        ...
        prob = np.array(self.model.predict(white)[0], dtype='float32')  #pass image to trained model and prob of 32
        ch1 = np.argmax(prob, axis=0)  # Most confident prediction
        prob[ch1] = 0
        ch2 = np.argmax(prob, axis=0)  # Second best prediction
        prob[ch2] = 0
        ch3 = np.argmax(prob, axis=0)   # Third best prediction
        prob[ch3] = 0
        ...
        if pl in l:
            if (self.pts[6][1] < self.pts[8][1] and self.pts[10][1] < self.pts[12][1] and self.pts[14][1] < self.pts[16][1] and self.pts[18][1] < self.pts[20][
                1]):#Index finger is bent,Middle finger is bent,Ring finger is bent,Pinky finger is bent
                ch1 = 0
        ...
        #Convert Predicted Index (ch1) into a Character
        if ch1 == 0:
            ch1 = 'S'
        ...
        self.prev_char=ch1  #Stores the recognized letter (ch1).
