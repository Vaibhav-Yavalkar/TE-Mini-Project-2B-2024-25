-- Add this at the beginning of the file to ensure MySQL user is created with mysql_native_password

-- Create user if it doesn't exist (commented out by default, uncomment to run)
-- CREATE USER IF NOT EXISTS 'root'@'127.0.0.1' IDENTIFIED WITH mysql_native_password BY 'Kisanjena@123';
...
CREATE TABLE IF NOT EXISTS health_data (
    ...
    height_feet INT NOT NULL DEFAULT 5,
    height_inches INT NOT NULL DEFAULT 0,
...
-- For backwards compatibility during migration, add this ALTER TABLE statement
-- This will be used once to migrate data, then can be commented out
ALTER TABLE health_data ADD COLUMN IF NOT EXISTS height_feet INT DEFAULT 5;
ALTER TABLE health_data ADD COLUMN IF NOT EXISTS height_inches INT DEFAULT 0;

-- Update any existing records to convert meters to feet/inches
UPDATE health_data 
SET 
    height_feet = FLOOR((height * 39.37) / 12),
    height_inches = FLOOR((height * 39.37) % 12)
WHERE height IS NOT NULL;

-- Add this line to eventually remove the height column after migration is complete
-- NOTE: Comment this out initially until migration is verified!
-- ALTER TABLE health_data DROP COLUMN height;
