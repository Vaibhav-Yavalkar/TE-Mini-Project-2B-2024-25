...
function AddNewInterview(){
    ...
    const onSubmit=async(e)=>{
        ...
        try {
            ...
            if (!result?.response) {
              console.error("Error: No response from AI model.");
              return;
            }
            ...
            try {
              const parsedJson = JSON.parse(MockJsonResp); // ✅ Proper JSON parsing
   ...
   // }
    ...
    const resp = await db.insert(MockInterview).values({
      ...
      if (resp?.length > 0) {
        console.log("Inserted Interview ID:", resp[0].mockId);
        setOpenDialog(false);
        router.push(`/dashboard/interview/${resp[0].mockId}`);
      } else {
