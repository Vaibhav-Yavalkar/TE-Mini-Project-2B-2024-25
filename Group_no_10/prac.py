...
app.config['MYSQL_PASSWORD'] = 'Yash@2411'  # Replace with your MySQL root password
...
try:
    ...
    nlp_skills = spacy.load('job1/datasetmodel')  # Use relative path
...
def load_and_filter_dataset(job_title):
    ...
    if 'job_descriptions' not in df_cache:
        with df_cache_lock:
            if 'job_descriptions' not in df_cache:
                df_cache['job_descriptions'] = pd.read_csv('job1/job_descriptions.csv')
...
def load_competition_dataset(job_title):
    ...
    if 'preprocessed_dataset' not in df_cache:
        with df_cache_lock:
            if 'preprocessed_dataset' not in df_cache:
                df_cache['preprocessed_dataset'] = pd.read_csv('job1/preprocessed_dataset.csv')
...
def csv_skills(text):
    ...
    skills_keywords = load_keywords('job1/newSkills.csv')
    skills = set()
...
job_data_path = "job1/models/job_data.pkl"
job_matrix_path = "job1/models/job_matrix.pkl"
tfidf_vectorizer_path = "job1/models/tfidf_vectorizer.pkl"
...
def analyze_salary_trends():
    ...
    try:
        ...
        df = pd.read_csv('job1/job_descriptions.csv')
...
ADZUNA_APP_ID = 'c2626ed5'
ADZUNA_APP_KEY = 'f7d7b8844d9f557080ba10c1e6248182'
ADZUNA_COUNTRY = 'us'
...
def analyze_job_competition():
    ...
    try:
        ...
        # Load dataset
        df = pd.read_csv("job1/preprocessed_dataset.csv")
...
def analyze_salary_data(job_title):
    try:
        # Format job title for file name (replace spaces with underscores)
        formatted_job_title = "_".join(job_title.strip().split()).title()
        model_filename = f"job1/models/salary_model_{formatted_job_title}.pkl"

        if not os.path.exists(model_filename):
            available_models = [f.replace('salary_model_', '').replace('.pkl', '').replace('_', ' ')
                              for f in os.listdir('job1/models')
                              if f.startswith('salary_model_') and f.endswith('.pkl')]
            return {
                'error': f'No salary data available for "{job_title}". Available job titles: {", ".join(available_models[:5])}...'
            }
